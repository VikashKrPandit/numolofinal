...
const recharge = async (req, res) => {
    ...
    if (recharge.length == 0) {
        ...
        const apiData = {
            key: "7f06415c-5495-4a1e-838d-00fca6b7f996",
            client_txn_id: client_transaction_id,
            amount: moneyString,
            p_info: 'Numolo Recharge',
            customer_name: userInfo.name_user,
            customer_email: userInfo.email,
            customer_mobile: mobile_no,
            redirect_url: "https://numolo.com/wallet/rechargerecord",
            udf1: 'Numolo',
        };
        console.log("create order ", apiData)
        ...
        try {
            ...
            if (apiResponse.data.status == true) {
                ...
                url = ?,
                time = ?`;
                await connection.execute(sql, [client_transaction_id, '0', userInfo.phone, userInfo.email, money, type, 0, checkTime, apiResponse.data.data.payment_url, timeNow]);
                const [recharge] = await connection.query('SELECT * FROM recharge WHERE phone = ? AND status = ? ', [userInfo.phone, 0]);
                return res.status(200).json({
                    message: 'Payment initiated successfully',
                    datas: recharge[0],
...
const addBank = async (req, res) => {
    ...
    const [user] = await connection.query('SELECT `phone`,`email`, `code`,`invite` FROM users WHERE `token` = ? ', [auth]);
    let userInfo = user[0];
    if (!user) {
        return res.status(200).json({
            message: 'Failed',
    ...
    const [user_bank2] = await connection.query('SELECT * FROM user_bank WHERE user_email = ? ', [userInfo.email]);
    if (user_bank.length == 0 && user_bank2.length == 0) {
        ...
        tinh = ?,
        time = ?`;
        await connection.execute(sql, [userInfo.phone, userInfo.email, name_bank, name_user, stk, email, tinh, time]);
        return res.status(200).json({
        ...
        await connection.query('UPDATE user_bank SET stk = ? WHERE email = ? ', [stk, userInfo.email]);
        return res.status(200).json({
        ...
        await connection.query('UPDATE user_bank SET name_bank = ?, name_user = ?, stk = ?, email = ?, tinh = ?, time = ? WHERE email = ?', [name_bank, name_user, stk, email, tinh, time, userInfo.email]);
        return res.status(200).json({
...
const withdrawal3 = async (req, res) => {
    ...
    const [user] = await connection.query('SELECT `phone`, `email`,`code`,`invite`, `money` FROM users WHERE `token` = ? AND password = ?', [auth, md5(password)]);
    ...
    const [recharge] = await connection.query('SELECT * FROM recharge WHERE email = ? AND status = 1', [userInfo.email]);
    const [minutes_1] = await connection.query('SELECT * FROM minutes_1 WHERE email = ?', [userInfo.email]);
    let total = 0;
    recharge.forEach((data) => {
        total += parseFloat(data.money);
    });
    ...
    result = Math.max(result, 0);
    const [user_bank] = await connection.query('SELECT * FROM user_bank WHERE `user_email` = ?', [userInfo.email]);
    const [withdraw] = await connection.query('SELECT * FROM withdraw WHERE `email` = ? AND today = ?', [userInfo.email, checkTime]);
        ...
        if (withdraw.length < 3) {
            if (userInfo.money - money >= 0) {
                ...
                if (!result == 0) {
                    ...
                    today = ?,
                    time = ?`;
                        await connection.execute(sql, [id_time + '' + id_order, userInfo.phone, userInfo.email, money, infoBank.stk, infoBank.name_bank, infoBank.email, infoBank.name_user, 0, checkTime, dates]);
                        await connection.query('UPDATE users SET money = money - ? WHERE email = ? ', [money, userInfo.email]);
                        return res.status(200).json({
...
const confirmRecharge = async (req, res) => {
    ...
    const [recharge] = await connection.query('SELECT * FROM recharge WHERE email = ? AND status = ? ', [userInfo.email, 0]);
    console.log('🚀 ~ recharge:', recharge);
    const [totalRechargeCount] = await connection.query('SELECT * FROM recharge WHERE email = ?', [userInfo.email]);

    if (recharge.length != 0) {
        const rechargeData = recharge[0];
        const timestamp = parseInt(rechargeData.time);
        ...
        try {
            ...
            if (apiRecord.client_txn_id === rechargeData.id_order && apiRecord.customer_email === rechargeData.email && apiRecord.amount === rechargeData.money) {
                if (apiRecord.status === 'success') {
                    // Check if this is the user's first recharge
                    if (recharge.length === 1) {
                        // This is the user's first recharge
                        // Calculate bonus based on recharge amount
                        let bonus = 0;
                        const rechargeAmount = apiRecord.amount;
                        if (rechargeAmount >= 100 && rechargeAmount <= 299) {
                            bonus = 30;
                        } else if (rechargeAmount >= 300 && rechargeAmount <= 999) {
                            bonus = 100;
                        } else if (rechargeAmount >= 1000 && rechargeAmount <= 9999) {
                            bonus = 150;
                        } else if (rechargeAmount >= 10000 && rechargeAmount <= 49999) {
                            bonus = 1000;
                        } else if (rechargeAmount >= 50000 && rechargeAmount <= 99999) {
                            bonus = rechargeAmount * 0.05;
                        } else if (rechargeAmount >= 100000) {
                            bonus = rechargeAmount * 0.10;
                        }

                        // Update user's balance with recharge amount and bonus
                        let totalAmount = 0;
                        if (totalRechargeCount.length > 1) {
                            totalAmount = rechargeAmount;
                        } else {
                            // totalAmount = rechargeAmount + bonus;
                            totalAmount = rechargeAmount;
                        }

                        await connection.query('UPDATE users SET money = money + ?, total_money = total_money + ? WHERE email = ? ', [totalAmount, totalAmount, apiRecord.customer_email]);

                        // Update recharge status
                        await connection.query(`UPDATE recharge SET status = 1 WHERE id = ? AND id_order = ? AND email = ? AND money = ?`, [rechargeData.id, apiRecord.client_txn_id, apiRecord.customer_email, apiRecord.amount]);
                        ...
                        // User has already made a recharge
                        return res.status(200).json({
                            message: 'Bonus only applicable for the first recharge',
                            status: false,
                            timeStamp: timeNow,
                        });
                    }
        ...
        return res.status(200).json({
            message: 'There are no pending recharges at the moment',
            status: false,
            timeStamp: timeNow,
        });
    }
