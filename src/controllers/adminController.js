...
const userInfo = async (req, res) => {
    ...
    const [user] = await connection.query('SELECT * FROM users WHERE email = ? ', [phone]);
    ...
    const [f1s] = await connection.query('SELECT `phone`,  `email`,`code`,`invite`, `time` FROM users WHERE `invite` = ? ', [userInfo.code]);
    ...
    for (let i = 0; i < f1s.length; i++) {
        ...
        const [f2s] = await connection.query('SELECT `phone`, `email`, `code`,`invite`, `time` FROM users WHERE `invite` = ? ', [f1_code]);
        for (let i = 0; i < f2s.length; i++) {
            ...
            const [f3s] = await connection.query('SELECT `phone`, `email`, `code`,`invite`, `time` FROM users WHERE `invite` = ? ', [f2_code]);
            for (let i = 0; i < f3s.length; i++) {
                const f3_code = f3s[i].code; // Mã giới thiệu f3
                const f3_time = f3s[i].time; // time f3
                let check_f3 = (timerJoin(f3_time) == timerJoin()) ? true : false;
                if (check_f3) f_all_today += 1;
                const [f4s] = await connection.query('SELECT `phone`, `email`, `code`,`invite`, `time` FROM users WHERE `invite` = ? ', [f3_code]);
    ...
    for (let i = 0; i < f1s.length; i++) {
        const f1_code = f1s[i].code; // Mã giới thiệu f1
        const [f2s] = await connection.query('SELECT `phone`, `email`, `code`,`invite` FROM users WHERE `invite` = ? ', [f1_code]);
        f2 += f2s.length;
    }
    ...
    for (let i = 0; i < f1s.length; i++) {
        ...
        for (let i = 0; i < f2s.length; i++) {
            ...
            const [f3s] = await connection.query('SELECT `phone`, `email`, `code`,`invite` FROM users WHERE `invite` = ? ', [f2_code]);
    ...
    for (let i = 0; i < f1s.length; i++) {
        ...
        for (let i = 0; i < f2s.length; i++) {
            ...
            for (let i = 0; i < f3s.length; i++) {
                ...
                const [f4s] = await connection.query('SELECT `phone`, `email`, `code`,`invite` FROM users WHERE `invite` = ? ', [f3_code]);
    ...
    const [recharge] = await connection.query('SELECT SUM(`money`) as total FROM recharge WHERE email = ? AND status = 1 ', [phone]);
    const [withdraw] = await connection.query('SELECT SUM(`money`) as total FROM withdraw WHERE email = ? AND status = 1 ', [phone]);
    const [bank_user] = await connection.query('SELECT * FROM user_bank WHERE user_email = ? ', [phone]);
    const [telegram_ctv] = await connection.query('SELECT `telegram` FROM point_list WHERE email = ? ', [userInfo.email]);
    const [ng_moi] = await connection.query('SELECT `email` FROM users WHERE code = ? ', [userInfo.invite]);
    return res.status(200).json({
        message: 'Success',
        status: true,
        datas: user,
...
const listRechargeMem = async (req, res) => {
    ...
    const [recharge] = await connection.query(`SELECT * FROM recharge WHERE email = ? ORDER BY id DESC LIMIT ${pageno}, ${limit} `, [phone]);
    const [total_users] = await connection.query(`SELECT * FROM recharge WHERE email = ?`, [phone]);
    return res.status(200).json({
        message: 'Success',
        status: true,
        datas: recharge,
...
const listWithdrawMem = async (req, res) => {
    ...
    const [withdraw] = await connection.query(`SELECT * FROM withdraw WHERE email = ? ORDER BY id DESC LIMIT ${pageno}, ${limit} `, [phone]);
    const [total_users] = await connection.query(`SELECT * FROM withdraw WHERE email = ?`, [phone]);
    return res.status(200).json({
        message: 'Success',
        status: true,
        datas: withdraw,
...
const listRedenvelope = async (req, res) => {
    ...
    const [redenvelopes_used] = await connection.query(`SELECT * FROM redenvelopes_used WHERE email = ? ORDER BY id DESC LIMIT ${pageno}, ${limit} `, [phone]);
    const [total_users] = await connection.query(`SELECT * FROM redenvelopes_used WHERE email = ?`, [phone]);
    return res.status(200).json({
        message: 'Success',
        status: true,
...
const listBet = async (req, res) => {
    ...
    const [listBet] = await connection.query(`SELECT * FROM minutes_1 WHERE email = ? AND status != 0 ORDER BY id DESC LIMIT ${pageno}, ${limit} `, [phone]);
    const [total_users] = await connection.query(`SELECT * FROM minutes_1 WHERE email = ? AND status != 0`, [phone]);
    return res.status(200).json({
        message: 'Success',
        status: true,
        datas: listBet,
