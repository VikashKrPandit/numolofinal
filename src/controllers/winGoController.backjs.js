...
const betWinGo = async (req, res) => {
    ...
    console.log("betwingo reqbody",req.body);
    let auth = req.cookies.auth;

    if (typeid != 1 && typeid != 3 && typeid != 5 && typeid != 10) {
        return res.status(200).json({
            message: 'Error!wingo1',
            status: true
        });
    }
    ...
    const [winGoNow] = await connection.query(`SELECT period FROM wingo WHERE status = 1 AND game = '${gameJoin}' ORDER BY id DESC LIMIT 1 `);
    const [user] = await connection.query('SELECT `phone`, `email`,`code`, `invite`, `level`, `money` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ', [auth]);
    console.log('winGonow',[winGoNow]);
    if (!winGoNow[0] || !user[0] || !isNumber(x) || !isNumber(money)) {
        return res.status(200).json({
            message: 'Error!wingo2',
            status: true
        });
    }
    ...
    today = ?,
    time = ?`;
        await connection.execute(sql, [id_product, userInfo.phone, userInfo.email, userInfo.code, userInfo.invite, period, userInfo.level, total, x, fee, 0, gameJoin, join, 0, checkTime, timeNow]);
...
const listOrderOld = async (req, res) => {
    console.log("game for 30 sec",req.body);
    let { typeid, pageno, pageto } = req.body;

    if (typeid != 1 && typeid != 3 && typeid != 5 && typeid != 10) {
        return res.status(200).json({
            message: 'Error!wingo3',
            status: true
        });
    }
...
const GetMyEmerdList = async (req, res) => {
    console.log("emerdListContler",req.body);
    let { typeid, pageno, pageto } = req.body;
    ...
    const [user] = await connection.query('SELECT `phone`, `email`,`code`, `invite`, `level`, `money` FROM users WHERE token = ? AND veri = 1 LIMIT 1 ', [auth]);
    const [minutes_1] = await connection.query(`SELECT * FROM minutes_1 WHERE email = ? AND game = '${game}' ORDER BY id DESC LIMIT ${Number(pageno) + ',' + Number(pageto)}`, [user[0].email]);
    const [minutes_1All] = await connection.query(`SELECT * FROM minutes_1 WHERE email = ? AND game = '${game}' ORDER BY id DESC `, [user[0].email]);
