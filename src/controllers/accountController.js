...
const login = async (req, res) => {
    console.log("login controller",res);
    ...
    try {
        const [rows] = await connection.query('SELECT * FROM users WHERE phone = ? AND password = ? ', [email, md5(pwd)]);
        if (rows.length == 1) {
            if (rows[0].status == 1) {
                ...
                await connection.execute('UPDATE `users` SET `token` = ? WHERE `phone` = ? ', [md5(accessToken), email]);
                return res.status(200).json({
...
/* const register = async (req, res) => {
    ...
    let { pwd, invitecode, email } = req.body;
    ...
    try {
        ...
        const [check_email] = await connection.query('SELECT * FROM users WHERE email = ?', [email]);
        ...
        if (check_email.length == 1 && check_email[0].veri == 1) {
            ...
            if (check_i.length == 1) {
                ...
                const sql = "INSERT INTO users SET id_user = ?,name_user = ?,password = ?, plain_password = ?, money = ?,code = ?,invite = ?,ctv = ?,veri = ?,otp = ?,ip_address = ?,status = ?,time = ?,email = ?";
                await connection.execute(sql, [id_user, name_user, md5(pwd), pwd, 0, code, invitecode, ctv, 1, otp2, ip, 1, time, email]);
                await connection.execute('INSERT INTO point_list SET email = ?', [email]);
...
const register = async(req, res) => {
    console.log('register_new_user',req.body);
    let now = new Date().getTime();
    let {email2, email, pwd, invitecode } = req.body;
    let id_user = randomNumber(10000, 99999);
    let otp2 = randomNumber(100000, 999999);
    let name_user = "Member" + randomNumber(10000, 99999);
    let code = randomString(5) + randomNumber(10000, 99999);
    ...
    if (!email2 || !email || !pwd || !invitecode) {
        return res.status(200).json({
    ...
    try {
        const [check_u] = await connection.query('SELECT * FROM users WHERE phone = ?', [email]);
        const [check_i] = await connection.query('SELECT * FROM users WHERE code = ? ', [invitecode]);
        const [check_ip] = await connection.query('SELECT * FROM users WHERE ip_address = ? ', [ip]);
        ...
        if (check_u.length == 1 && check_u[0].veri == 1) {
            ...
            if (check_i.length == 1) {
                if (check_ip.length <= 3) {
                    ...
                    const sql = "INSERT INTO users SET id_user = ?,phone = ?, email=?, name_user = ?,password = ?, plain_password = ?, money = ?,code = ?,invite = ?,ctv = ?,veri = ?,otp = ?,ip_address = ?,status = ?,time = ?";
                    await connection.execute(sql, [id_user, email, email2, name_user, md5(pwd), pwd, 0, code, invitecode, ctv, 1, otp2, ip, 1, time]);
                    await connection.execute('INSERT INTO point_list SET phone = ?', [email]);
                    return res.status(200).json({
